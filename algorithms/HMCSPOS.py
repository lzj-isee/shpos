...
class HMCSPOS(basic):
    def __init__(self, opts):
        ...
        save_name = algo_name + '_'+\
            'lr[{:.2e}]M[{}]b[{}]k_h[{:.1e}]w[{:.1e}]ge[{:.3f}]u[{:.2f}]s[{}]'.format(\
            opts.lr, opts.particle_num, opts.batch_size, opts.kernel_h, opts.weight, opts.ge, opts.u, opts.seed)
        self.gamma = opts.ge / opts.lr
        super(HMCSPOS, self).__init__(opts, algo_name, save_name)
            ...
            for iter, (train_features, train_labels) in enumerate(self.train_loader):
                ...
                for i in range(self.opts.particle_num):
                    v_list[i, :] += - self.gamma * eta * v_list[i, :] - eta * self.opts.u * grad_list[i, :] + noise_v[i, :] +\
                        ...
                        (kernel[:,i].view(-1,1) * (-grad_list - 2*(x_temp-x_temp[i,:].view(1,-1))/kernel_h)).mean(0)
                # evaluation
