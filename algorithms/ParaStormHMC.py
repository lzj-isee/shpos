...
class ParaStormHMC(basic):
    def __init__(self, opts):
        algo_name = 'ParaStormHMC'
        save_name = algo_name + '_'+\
            'lr[{:.2e}]M[{}]b[{}]ge[{:.3f}]u[{:.2f}]rp[{},{:.1f}]s[{}]'.format(\
            opts.lr, opts.particle_num, opts.batch_size, opts.ge, opts.u, 
            opts.refresh_value, opts.pow, opts.seed)
        self.gamma = opts.ge / opts.lr
        super(ParaStormHMC, self).__init__(opts, algo_name, save_name)
